...
'.$content.'
        ...
        protected function _calculateTableCellSize(&$cases, &$corr)
        {
            ...
            // for each cell without counitan, we get the max width for each column
            $sw = array();
            ...
            // for each cell with counitan, we adapt the width of each column
            for ($x=0; $x<count($corr[0]); $x++) {
                for ($y=0; $y<count($corr); $y++) {
                    if (isset($corr[$y][$x]) && is_array($corr[$y][$x]) && $corr[$y][$x][2]>1) {

                        // sum the max width of each column in counitan
                        $s = 0; for ($i=0; $i<$corr[$y][$x][2]; $i++) $s+= $sw[$x+$i];

                        // if the max width is < the width of the cell with counitan => we adapt the width of each max width
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
            ...
            if ($this->_subPart) {
                ...
                HTML2PDF::$_tables[$param['num']]['celunitacing']     = $this->parsingCss->ConvertToMM(isset($param['celunitacing']) ? $param['celunitacing'] : '2px');
                HTML2PDF::$_tables[$param['num']]['cases']           = array();          // properties of each TR/TD
                HTML2PDF::$_tables[$param['num']]['corr']            = array();          // link between TR/TD and counitan/rowspan
                ...
                HTML2PDF::$_tables[$param['num']]['marge']['t']      = $this->parsingCss->value['padding']['t']+$this->parsingCss->value['border']['t']['width']+HTML2PDF::$_tables[$param['num']]['celunitacing']*0.5;
                HTML2PDF::$_tables[$param['num']]['marge']['r']      = $this->parsingCss->value['padding']['r']+$this->parsingCss->value['border']['r']['width']+HTML2PDF::$_tables[$param['num']]['celunitacing']*0.5;
                HTML2PDF::$_tables[$param['num']]['marge']['b']      = $this->parsingCss->value['padding']['b']+$this->parsingCss->value['border']['b']['width']+HTML2PDF::$_tables[$param['num']]['celunitacing']*0.5;
                HTML2PDF::$_tables[$param['num']]['marge']['l']      = $this->parsingCss->value['padding']['l']+$this->parsingCss->value['border']['l']['width']+HTML2PDF::$_tables[$param['num']]['celunitacing']*0.5;
                ...
                if (!$this->_isInTfoot && HTML2PDF::$_tables[$param['num']]['td_y'] + HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty +$hfoot> $this->pdf->getH() - $this->pdf->getbMargin()) {
                    ...
                    if (isset(HTML2PDF::$_tables[$param['num']]['height'][HTML2PDF::$_tables[$param['num']]['page']])) {
                        ...
                        $this->_drawRectangle(
                            ...
                            $this->parsingCss->value['border'],
                            $this->parsingCss->value['padding'],
                            HTML2PDF::$_tables[$param['num']]['celunitacing']*0.5,
                            $this->parsingCss->value['background']
                        );
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            $param['celunitacing'] = HTML2PDF::$_tables[$param['num']]['celunitacing'].'mm';
            ...
            $counitan = isset($param['counitan']) ? $param['counitan'] : 1;
            ...
            if (in_array($other, array('td', 'th'))) {
                ...
                if (isset(HTML2PDF::$_tables[$param['num']]['cols'][$numCol])) {
                    ...
                    // for counitans => we get all the neede widths
                    $colParam['style']['width'] = array();
                    for ($k=0; $k<$counitan; $k++) {
            ...
            $marge = array();
            $marge['t'] = $this->parsingCss->value['padding']['t']+0.5*HTML2PDF::$_tables[$param['num']]['celunitacing']+$this->parsingCss->value['border']['t']['width'];
            $marge['r'] = $this->parsingCss->value['padding']['r']+0.5*HTML2PDF::$_tables[$param['num']]['celunitacing']+$this->parsingCss->value['border']['r']['width'];
            $marge['b'] = $this->parsingCss->value['padding']['b']+0.5*HTML2PDF::$_tables[$param['num']]['celunitacing']+$this->parsingCss->value['border']['b']['width'];
            $marge['l'] = $this->parsingCss->value['padding']['l']+0.5*HTML2PDF::$_tables[$param['num']]['celunitacing']+$this->parsingCss->value['border']['l']['width'];
            ...
            if ($this->_subPart) {
                ...
                HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['counitan'] = $counitan;
                ...
                // prepare the mapping for rowspan and counitan
                for ($j=0; $j<$rowspan; $j++) {
                    for ($i=0; $i<$counitan; $i++) {
                        HTML2PDF::$_tables[$param['num']]['corr']
                            [HTML2PDF::$_tables[$param['num']]['corr_y']+$j]
                            [HTML2PDF::$_tables[$param['num']]['corr_x']+$i] = ($i+$j>0) ? '' : array($x,$y,$counitan,$rowspan);
                    }
                }
                HTML2PDF::$_tables[$param['num']]['corr_x']+= $counitan;
                while (isset(HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y']][HTML2PDF::$_tables[$param['num']]['corr_x']])) {
                    HTML2PDF::$_tables[$param['num']]['corr_x']++;
                }
