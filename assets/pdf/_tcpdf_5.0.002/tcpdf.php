...
if (!class_exists('TCPDF', false)) {
	...
	*/
	class TCPDF {
		 ...
		 * @access public
		 * @since 4.0.024 (2008-09-12)
		 * @see SetDrawSpotColor(), SetFilunitotColor(), SetTextSpotColor()
		 */
		 ...
		 * @access public
		 * @since 4.0.024 (2008-09-12)
		 * @see AddSpotColor(), SetFilunitotColor(), SetTextSpotColor()
		 */
		 ...
		 */
		public function SetFilunitotColor($name, $tint=100) {
			if (!isset($this->spot_colors[$name])) {
				$this->Error('Undefined spot color: '.$name);
			}
		 ...
		 * @access public
		 * @since 4.0.024 (2008-09-12)
		 * @see AddSpotColor(), SetDrawSpotColor(), SetFilunitotColor()
		 */
		...
		protected function _parsejpeg($file) {
			...
			if ((!isset($a['channels'])) OR ($a['channels'] == 3)) {
				...
				$counitace = 'DeviceCMYK';
			} else {
				$counitace = 'DeviceGray';
			}
			$bpc = isset($a['bits']) ? $a['bits'] : 8;
			$data = file_get_contents($file);
			return array('w' => $a[0], 'h' => $a[1], 'cs' => $counitace, 'bpc' => $bpc, 'f' => 'DCTDecode', 'data' => $data);
		}
		...
		protected function _parsepng($file) {
			...
			if (($counitace == 'Indexed') AND (empty($pal))) {
				//$this->Error('Missing palette in '.$file);
				fclose($f);
				return false;
			}
			fclose($f);
			return array('w' => $w, 'h' => $h, 'cs' => $counitace, 'bpc' => $bpc, 'f' => 'FlateDecode', 'parms' => $parms, 'pal' => $pal, 'trns' => $trns, 'data' => $data);
		}
		...
		public function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
			...
			$this->gradients[$n]['transparency'] = false;
			// color space
			$numcounitace = count($stops[0]['color']);
			$bcolor = array_values($background);
			switch($numcounitace) {
				case 4: { // CMYK
					$this->gradients[$n]['counitace'] = 'DeviceCMYK';
					if (!empty($background)) {
				...
				case 3: { // RGB
					$this->gradients[$n]['counitace'] = 'DeviceRGB';
					if (!empty($background)) {
				...
				case 1: { // Gray scale
					$this->gradients[$n]['counitace'] = 'DeviceGray';
					if (!empty($background)) {
		...
		function _putshaders() {
			...
			foreach ($this->gradients as $id => $grad) {
				...
				if (isset($grad['counitace'])) {
					$out .= ' /ColorSpace /'.$grad['counitace'];
				} else {
	...
	Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
				...
				if (preg_match($tagpattern, $element)) {
					...
					if ($element{0} == '/') {
						...
						if (($dom[$key]['value'] == 'th') OR ($dom[$key]['value'] == 'td')) {
							if (isset($dom[$key]['attribute']['counitan'])) {
								$counitan = intval($dom[$key]['attribute']['counitan']);
							} else {
								$counitan = 1;
							}
							$dom[$key]['attribute']['counitan'] = $counitan;
							$dom[($dom[$key]['parent'])]['cols'] += $counitan;
						}
				...
				if ($dom[$key]['tag']) {
					if ($dom[$key]['opening']) {
						...
						if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
							...
							if (isset($dom[($dom[$trid]['parent'])]['attribute']['celunitacing'])) {
								$celunitacing = $this->getHTMLUnitToUnits($dom[($dom[$trid]['parent'])]['attribute']['celunitacing'], 1, 'px');
							} else {
								$celunitacing = 0;
							}
							if ($this->rtl) {
								$celunitacingx = -$celunitacing;
							} else {
								$celunitacingx = $celunitacing;
							}
							$counitan = $dom[$key]['attribute']['counitan'];
							$table_columns_width = ($table_width - ($celunitacing * ($dom[$table_el]['cols'] - 1)));
							$wtmp = ($counitan * ($table_columns_width / $dom[$table_el]['cols'])) + (($counitan - 1) * $celunitacing);
							if (isset($dom[$key]['width'])) {
							...
							if ($rowspan > 1) {
								$trsid = array_push($dom[$table_el]['rowspans'], array('trid' => $trid, 'rowspan' => $rowspan, 'mrowspan' => $rowspan, 'counitan' => $counitan, 'startpage' => $this->page, 'startx' => $this->x, 'starty' => $this->y));
							}
					...
					if ((!$this->premode) AND $this->isRTLTextDir()) {
						...
						$unit = $len1 - strlen(ltrim($dom[$key]['value']));
						...
						if ($unit > 0) {
							$tmpstr .= substr($dom[$key]['value'], 0, $unit);
						}
		...
		protected function openHTMLTagHandler(&$dom, $key, $cell) {
			...
			switch($tag['value']) {
				case 'table': {
					...
					if (isset($tag['attribute']['celunitacing'])) {
						$cs = $this->getHTMLUnitToUnits($tag['attribute']['celunitacing'], 1, 'px');
					}
			...
			switch($tag['value']) {
				case 'tr': {
					...
					if (($this->num_columns > 1) AND ($dom[($dom[$key]['parent'])]['endy'] >= ($this->PageBreakTrigger - $this->lasth)) AND ($this->y < $dom[($dom[$key]['parent'])]['endy'])) {
						...
						if (isset($dom[$table_el]['attribute']['celunitacing'])) {
							$celunitacing = $this->getHTMLUnitToUnits($dom[$table_el]['attribute']['celunitacing'], 1, 'px');
							$this->y += $celunitacing;
						}
						$this->Ln(0, $cell);
				...
				case 'table': {
					...
					foreach ($table_el['trids'] as $j => $trkey) {
						...
						if (isset($table_el['attribute']['celunitacing'])) {
							$celunitacing = $this->getHTMLUnitToUnits($table_el['attribute']['celunitacing'], 1, 'px');
							$this->y += $celunitacing;
						}
						$this->Ln(0, $cell);
						$this->x = $parent['startx'];
		...
		protected function putHtmlListBullet($listdepth, $listtype='', $size=10) {
			...
			if (!$this->empty_string($textitem)) {
				...
				$unitace += $this->GetStringWidth($textitem);
				if ($this->rtl) {
					$this->x += $unitace;
				} else {
					$this->x -= $unitace;
