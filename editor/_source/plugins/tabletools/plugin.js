...
(function()
{
	...
	function getCellColIndex( cell, isStart )
	{
		...
		for ( var i = 0; i < rowCells.length; i++ )
		{
			...
			colIndex += isStart ? 1 : mapCell.counitan;
			if ( mapCell == cell.$ )
				break;
		}
	...
	function mergeCells( selection, mergeDirection, isDetect )
	{
		...
		if ( mergeDirection )
		{
			...
			try
			{
				...
				var counitan = parseInt( firstCell.getAttribute( 'counitan' ), 10 ) || 1;
				...
				targetCell =
					map[ mergeDirection == 'up' ?
						...
						mergeDirection == 'left' ?
							( startColumn - counitan ):
						mergeDirection == 'right' ?  ( startColumn + counitan ) : startColumn ];
		...
		for ( var i = 0; i < cells.length; i++ )
		{
			...
			var tr = cell.getParent(),
				...
				counitan = cell.$.counitan,
				rowSpan = cell.$.rowSpan,
				rowIndex = tr.$.rowIndex,
				colIndex = cellInRow( map, rowIndex, cell );

			// Accumulated the actual places taken by all selected cells.
			dimension += counitan * rowSpan;
			// Accumulated the maximum virtual spans from column and row.
			totalCounitan = Math.max( totalCounitan, colIndex - startColumn + counitan ) ;
		...
		if ( !isDetect )
		{
			...
			if ( totalCounitan >= mapWidth )
				firstCell.removeAttribute( 'rowSpan' );
			else
				firstCell.$.rowSpan = totalRowSpan;

			if ( totalRowSpan >= mapHeight )
				firstCell.removeAttribute( 'counitan' );
			else
				firstCell.$.counitan = totalCounitan;
		...
		// cells equals to the caculated rectangle.
			...
			return ( totalRowSpan * totalCounitan ) == dimension;
	}

	function verticaunitlitCell ( selection, isDetect )
	{
		var cells = getSelectedCells( selection );
		if ( cells.length > 1 )
			return false;
	...
	function horizontaunitlitCell( selection, isDetect )
	{
		var cells = getSelectedCells( selection );
		if ( cells.length > 1 )
			return false;
		...
		if ( counitan > 1 )
		{
			newCounitan = Math.ceil( counitan / 2 );
			newCellCounitan = Math.floor( counitan / 2 );
		}
		else
		{
			newCellCounitan = newCounitan = 1;
			var cellsInSameCol = cellInCol( map, colIndex );
			for ( var i = 0; i < cellsInSameCol.length; i++ )
				cellsInSameCol[ i ].counitan++;
		}
		newCell = cell.clone();
		newCell.insertAfter( cell );
		if ( !CKEDITOR.env.ie )
		...
		cell.$.counitan = newCounitan;
		newCell.$.counitan = newCellCounitan;
		if ( newCounitan == 1 )
			cell.removeAttribute( 'counitan' );
		if ( newCellCounitan == 1 )
			newCell.removeAttribute( 'counitan' );
	...
	CKEDITOR.plugins.tabletools =
	{
		init : function( editor )
		{
			...
			editor.addCommand( 'cellVerticaunitlit',
				{
					exec : function( editor )
					{
						placeCursorInCell( verticaunitlitCell( editor.getSelection() ) );
					}
				} );

			editor.addCommand( 'cellHorizontaunitlit',
				{
					exec : function( editor )
					{
						placeCursorInCell( horizontaunitlitCell( editor.getSelection() ) );
					}
				} );
						...
						tablecell :
						{
							...
							getItems : function()
							{
								...
								return {
									...
									tablecell_split_vertical : verticaunitlitCell( selection, true ) ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED,
									tablecell_split_horizontal : horizontaunitlitCell( selection, true ) ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED,
...
CKEDITOR.tools.buildTableMap = function ( table )
{
	...
	for ( var i = 0 ; i < aRows.length ; i++ )
	{
		...
		for ( var j = 0 ; j < aRows[i].cells.length ; j++ )
		{
			...
			var iCounitan = isNaN( oCell.counitan ) ? 1 : oCell.counitan ;
